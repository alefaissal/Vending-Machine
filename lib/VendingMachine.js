class VendingMachine {
  ...
  order(code, money) {
    ...
    if (itemQty < 1) {
      ...
      //Next line update inventory when sell a product
      this.inventory.items.map(item =>
        item.id === code ? item.current-- : ""
      );
  ...
  refilCoins(inventory) {
    inventory.coins = inventory.coins.map = type => {
      type.current = type.need;
    };
    return inventory;
  }
}
